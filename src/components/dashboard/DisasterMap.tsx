...
const DisasterMap = () => {
  ...
  const { selectedEventId, disasters, fullscreenAlerts, setFullscreenAlerts } = useDisasterContext();
    ...
    >
      ...
      {!fullscreenAlerts && (
        <button
          ...
          title="Open fullscreen map + alerts"
          className="absolute top-3 right-3 z-[1000] flex items-center gap-1.5 bg-card/90 backdrop-blur-sm rounded px-3 py-1.5 border border-border text-muted-foreground hover:text-primary hover:border-primary/50 transition-colors"
        >
          <Maximize2 className="h-3.5 w-3.5" />
          <span className="text-[10px] font-mono uppercase tracking-wider">Full Screen</span>
        </button>
      )}
